...
def add_gen_mass_logits(h5_path, mass_classes=None, output_path=None, inplace=True):
    ...
    try:
        ...
        with h5py.File(h5_path, 'r') as f_in:
            ...
            # Store continuous mass values as gen_mass_logits
            # The code expects continuous mass values (like gen_mass), then converts them to class indices
            # The name "logits" is misleading - these are actually continuous mass values
            # The code will convert them to class indices by finding the nearest mass class
            gen_mass_logits = mass_data.astype(np.float32)  # Just use the mass values directly
            print(f"Generated gen_mass_logits shape: {gen_mass_logits.shape}")
            print(f"gen_mass_logits range: [{gen_mass_logits.min():.2f}, {gen_mass_logits.max():.2f}] (continuous mass values)")
            ...
            with h5py.File(temp_path, 'w') as f_out:
                ...
                print(f"  Sample values (first 5 samples):")
                for i in range(min(5, len(gen_mass_logits))):
                    mass_val = float(gen_mass_logits[i])
                    # Find closest class
                    distances = np.abs(mass_val - np.array(mass_classes))
                    closest_idx = np.argmin(distances)
                    print(f"    Sample {i}: mass={mass_val:.2f}, closest_class={mass_classes[closest_idx]}")
